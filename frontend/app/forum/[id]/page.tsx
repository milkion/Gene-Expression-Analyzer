...
export default function PostDetailPage() {
  ...
  // Fetch post details
  const { loading, error: queryError, data, refetch } = useQuery(GET_FORUM_POST, {
    variables: { id: postId },
  ...
  if (queryError) return <p className="p-8 text-center text-red-500">Error: {queryError.message}</p>;
  if (!data?.forumPost) return <p className="p-8 text-center">Post not found</p>;
      ...
      <div>
        ...
        <div className="p-4 mx-8">
          <div className="flex items-center gap-2 text-gray-600 mt-4 mx-4">
            <span className="cursor-pointer hover:underline" onClick={() => router.push("/forum")}>Forum</span>
            <span>{">"}</span>
            <span>{post.title}</span>
          </div>
          
          <div className="bg-slate-100 min-h-screen rounded-3xl py-10 px-10 mt-6">
            {/* Post Header */}
                    ...
                    <div key={comment.id} className="bg-white rounded-xl p-6 shadow-sm">
                      <div className="flex justify-between items-start">
                        ...
                        <button
                          ...
                          className={`text-gray-400 hover:text-red-500 ${currentUserId !== comment.author.id ? 'hidden' : ''}`}
                        >
